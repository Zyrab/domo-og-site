...
export default async function createDocsPage() {
  ...
  const headings = lines
    ...
    .map(l => {
      ...
      const id = text.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^\w-]+/g, "").replace(/--+/g, "-");
      return { level, text, id };
    })
    .filter(h => h && h.level <= 3);

  const sidebarTitle = Domo("div").cls("sidebar-title uppercase bold mb-1").txt("Table of Contents").build();
  ...
  const setupScrollSpy = (el) => {
    ...
    el.querySelectorAll('.docs-content h1, .docs-content h2, .docs-content h3').forEach(h => {
      if (!h.id) {
        h.id = h.textContent.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^\w-]+/g, "").replace(/--+/g, "-");
      }
      observer.observe(h);
    });
  };
  ...
  return Domo("div")
    .cls("docs-page")
    .child([
      Domo("div").cls("docs-layout container").child([
    ...
    .id("docs-page")
    .onClosest("click", { ".copy": copyCode }).ref(setupScrollSpy);
}
